...
class Didar_Admin {
	...
	public function render_didar_field_mappings_selector() { $s = $this->settings->all(); $defaults = new Didar_Field_Mapper( $this->registry, $this->settings ); echo '<script type="application/json" id="didar-custom-field-catalog">' . wp_json_encode( array( 'fields' => $this->workflow->custom_fields(), 'pipelines' => $this->workflow->pipelines() ), JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES ) . '</script><p>فیلد دیدار از کاستوم‌فیلدهای معامله و بر اساس کاریز انتخاب‌شده برای همین فرم تعیین می‌شود.</p>'; foreach ( $this->registry->all() as $form_type => $form ) { $pipeline_id = $this->workflow->workflow( $form_type )['pipeline_id'] ?? ''; echo '<div class="didar-field-mapping-form" data-didar-field-mapping="' . esc_attr( $form_type ) . '"><h3>' . esc_html( $form['label'] ) . '</h3><table class="widefat striped"><tbody>'; foreach ( $this->registry->fields( $form_type ) as $key => $field ) { if ( ! empty( $field['internal'] ) || 'honeypot' === $field['type'] ) { continue; } $map = $s['didar_field_mappings'][ $form_type ][ $key ] ?? $defaults->mapping( $form_type, $key ); $base = Didar_Settings::OPTION_NAME . '[didar_field_mappings][' . $form_type . '][' . $key . ']'; echo '<tr><td><strong>' . esc_html( $field['label'] ) . '</strong><br><code>' . esc_html( $key ) . '</code></td><td><input type="hidden" name="' . esc_attr( $base . '[target]' ) . '" value="deal_custom">'; $this->render_custom_field_select( $base . '[field]', $map['field'] ?? '', $pipeline_id ? array( $pipeline_id ) : array() ); echo '</td></tr>'; } echo '</tbody></table></div>'; } }
