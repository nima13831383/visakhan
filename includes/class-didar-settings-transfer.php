...
class Didar_Settings_Transfer {
	...
	private function portable_option_keys() {
		return array( 'visa_companion_case_settings', 'case_form_settings', 'didar_form_workflows', 'didar_field_mappings', 'didar_form_field_placeholders', 'didar_form_access', 'didar_broker_user_map', 'didar_form_default_assignees', 'didar_default_owner_id', 'didar_default_pipeline_id', 'didar_system_form_type_field_id', 'didar_system_submission_id_field_id', 'didar_system_user_id_field_id', 'didar_public_status_field_id', 'field_required_overrides', 'profile_field_states', 'didar_user_person_mappings', 'didar_notification_events', 'colleague_can_view_internal_history', 'frontend_requests_per_page', 'file_download_mode', 'didar_debug_logging', 'pdf_settings' );
	}
	...
	public function portable_settings( $source = null ) {
		...
		$out['didar_form_access'] = isset( $out['didar_form_access'] ) && is_array( $out['didar_form_access'] ) ? $this->normalize_form_access( $out['didar_form_access'] ) : array();
		...
		$out['pdf_settings'] = Didar_Settings::normalize_pdf_settings( $out['pdf_settings'] ?? array() );
		$out['didar_notification_events'] = Didar_Notification_Event_Registry::normalize_configuration( $out['didar_notification_events'] ?? array() );
	...
	private function normalize( $raw, &$warnings, &$errors ) {
		$out = array(); $allowed = array_keys( $this->portable_settings( array_fill_keys( array( 'visa_companion_case_settings','case_form_settings','didar_form_workflows','didar_field_mappings','didar_form_field_placeholders','didar_form_access','didar_broker_user_map','didar_form_default_assignees','didar_default_owner_id','didar_default_pipeline_id','didar_system_form_type_field_id','didar_system_submission_id_field_id','didar_system_user_id_field_id','didar_public_status_field_id','field_required_overrides','profile_field_states','didar_user_person_mappings','didar_notification_events','colleague_can_view_internal_history','frontend_requests_per_page','file_download_mode','didar_debug_logging','pdf_settings' ), null ) ) );
		...
		foreach ( array( 'didar_form_workflows','didar_field_mappings','didar_form_field_placeholders','field_required_overrides','profile_field_states','didar_user_person_mappings','pdf_settings' ) as $key ) { if ( isset( $out[$key] ) && ! is_array( $out[$key] ) ) { $errors[] = 'ساختار «' . $key . '» معتبر نیست.'; unset( $out[$key] ); } }
		...
		if ( isset( $out['didar_form_access'] ) && ! is_array( $out['didar_form_access'] ) ) { $errors[] = 'ساختار لینک و بارکد فرم‌ها معتبر نیست.'; unset( $out['didar_form_access'] ); }
		if ( isset( $out['didar_form_access'] ) ) { $out['didar_form_access'] = $this->normalize_form_access( $out['didar_form_access'], $warnings ); }
		if ( isset( $out['pdf_settings'] ) ) { $out['pdf_settings'] = Didar_Settings::normalize_pdf_settings( $out['pdf_settings'] ); }
		if ( isset( $out['didar_notification_events'] ) ) {
			$out['didar_notification_events'] = Didar_Notification_Event_Registry::normalize_configuration( $out['didar_notification_events'] );
			foreach ( $out['didar_notification_events'] as $event_key => $event_configuration ) {
				...
				if ( empty( $validation['valid'] ) ) { $errors[] = 'قالب Email برای رویداد «' . sanitize_text_field( (string) $event_key ) . '» معتبر نیست.'; }
			}
		}
	...
	private function portable_defaults() { return array( 'didar_form_workflows'=>array(),'didar_field_mappings'=>array(),'didar_form_field_placeholders'=>array(),'didar_form_access'=>array(),'didar_broker_user_map'=>array(),'didar_form_default_assignees'=>array(),'didar_default_owner_id'=>'','didar_default_pipeline_id'=>'','didar_system_form_type_field_id'=>'','didar_system_submission_id_field_id'=>'','didar_system_user_id_field_id'=>'','didar_public_status_field_id'=>'','field_required_overrides'=>array(),'profile_field_states'=>Didar_Settings::PROFILE_FIELD_STATES,'didar_user_person_mappings'=>array(),'didar_notification_events'=>Didar_Notification_Event_Registry::default_configuration(),'colleague_can_view_internal_history'=>0,'frontend_requests_per_page'=>Didar_Settings::DEFAULT_REQUESTS_PER_PAGE,'file_download_mode'=>Didar_Settings::DEFAULT_FILE_DOWNLOAD_MODE,'didar_debug_logging'=>'off','pdf_settings'=>Didar_Settings::normalize_pdf_settings( array() ) ); }
	...
	private function normalize_form_access( $value, &$warnings = array() ) {
		$out = array();
		foreach ( (array) $value as $form_type => $entry ) {
			$form_type = sanitize_key( (string) $form_type );
			if ( ! $form_type || ! $this->registry->get( $form_type ) || ! is_array( $entry ) ) {
				if ( $form_type ) { $warnings[] = 'تنظیم لینک و بارکد برای فرم ناشناخته یا نامعتبر نادیده گرفته شد.'; }
				continue;
			}
			$url     = Didar_Form_Access::sanitize_url( $entry['url'] ?? '' );
			$barcode = Didar_Form_Access::sanitize_image_url( $entry['barcode'] ?? '' );
			if ( ! empty( $entry['url'] ) && ! $url ) { $warnings[] = 'نشانی فرم دیدار برای «' . $form_type . '» معتبر نیست و وارد نشد.'; }
			if ( ! empty( $entry['barcode'] ) && ! $barcode ) { $warnings[] = 'نشانی تصویر بارکد برای «' . $form_type . '» معتبر نیست و وارد نشد.'; }
		 ...
		 switch ( $option_name ) {
			...
			case 'didar_form_field_defaults':
				...
				foreach ( (array) $value as $form_type => $fields ) {
					...
					foreach ( (array) $fields as $field_key => $source ) {
						$field_key = sanitize_key( (string) $field_key ); $source = sanitize_key( (string) $source );
						if ( ! $source ) { continue; }
						if ( ! empty( $definitions[ $field_key ]['default_value_options'] ) && isset( $definitions[ $field_key ]['options'][ $source ] ) ) { $out[ $form_type ][ $field_key ] = $source; }
						elseif ( in_array( $source, $catalog->keys(), true ) ) { $out[ $form_type ][ $field_key ] = $source; }
					}
				}
			...
			case 'didar_form_access':
				$out = array();
				foreach ( (array) $value as $form_type => $entry ) {
					$form_type = sanitize_key( (string) $form_type );
					if ( ! $form_type || ! is_array( $entry ) ) { continue; }
					$out[ $form_type ] = Didar_Form_Access::normalize_entry( $entry );
				}
				return $this->sort_associative( $out );
	...
	private function diff($old,$new){$groups=array('گردش کار فرم‌ها'=>array('didar_form_workflows'),'نگاشت فیلدهای فرم'=>array('didar_field_mappings'),'کاربران دیدار'=>array('didar_broker_user_map','didar_default_owner_id'),'فیلدهای سیستمی'=>array('didar_system_form_type_field_id','didar_system_submission_id_field_id','didar_system_user_id_field_id'),'تنظیمات قدیمی وضعیت'=>array('didar_public_status_field_id'),'فرم اطلاعات کاربری'=>array('profile_field_states','didar_user_person_mappings'),'چاپ درخواست'=>array('pdf_settings'),'سایر تنظیمات'=>array('didar_form_access','field_required_overrides','colleague_can_view_internal_history','frontend_requests_per_page','file_download_mode','didar_debug_logging','didar_default_pipeline_id'));$out=array();foreach($groups as $name=>$keys){$changed=0;$same=0;foreach($keys as $k){if(!array_key_exists($k,$new))continue;if(($old[$k]??null)===$new[$k])$same++;else $changed++;}$out[$name]=array('added'=>0,'changed'=>$changed,'unchanged'=>$same,'invalid'=>0);}return $out;}
